...
void HandleVotes() {
  ...
  float votePercentCTs = 0;
  float votePercentTs = 0;

  votePercentCTs = g_iVoteCTs / GetNumHumansOnTeam(CS_TEAM_CT);
  LogMessage("Vote percentage CT %f", votePercentCTs);
  votePercentTs = g_iVoteTs / GetNumHumansOnTeam(CS_TEAM_T);
